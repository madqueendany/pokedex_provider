...
class PageManager with ChangeNotifier {
  ViewStyle _style = ViewStyle.grid;

  ViewStyle get style => _style;

  void setStyle(ViewStyle vs) {
    _style = vs;
    notifyListeners();
  }
}
...
class PokdexAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SliverAppBar(
      ...
      actions: <Widget>[
        Consumer<PageManager>(builder: (_, provider, __) {
          return IconButton(
            icon: Icon(
              provider.style == ViewStyle.grid
                  ? Icons.table_chart
                  : Icons.grid_on,
            ),
            color: Colors.black87,
            onPressed: () {
              provider.setStyle(provider.style == ViewStyle.grid
                  ? ViewStyle.list
                  : ViewStyle.grid);
            },
          );
    ...
    return StreamProvider<PokedexListModel>(
      ...
      child: Consumer<PokedexListModel>(
        builder: (context, provider, child) {
          ...
          if (Provider.of<PageManager>(context).style == ViewStyle.list)
            return SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) =>
